	...
	public static void main(String[] args) {
		...
		while (run) {
			...
			if (menu == 1) {
				...
				checkMember.setMno(mno);
				boolean mbTable = gdao.selectMno(checkMember);
				...
				List<Member> list = gdao.getList(null, mno);
				for (Member mb1 : list) {
					System.out.println("===================");
				...
				while (true) {
					...
					for (Member mb : list) {
						System.out.println("    " + mb.getMno() + "      " + mb.getName() + "     " + mb.getGender()
								+ "      " + mb.getAddress() + "     " + mb.getPhone());
				...
				int mno = scn.nextInt();
				scn.nextLine();
				// 입력받은 회원번호를 Member 객체에 설정
				mb.setMno(mno);

				System.out.println("1.이메일 2.거주지 3.전화번호");
				int menu2 = scn.nextInt();
				scn.nextLine();
				// 선택된 메뉴에 따라 해당 정보를 Member 객체에 설정
				if (menu2 == 1) {
				...
				//각 이용권 옵션별 매출 조회 및 출력
				String[] options = {"1개월", "3개월", "6개월", "12개월"};
			    for (String option : options) {
			        int salesByOption = gdao.getSalesByOption(option);
			        if (salesByOption != -1) {
			            System.out.println(option + " 매출: " + salesByOption + "원");
			        } else {
			            System.out.println(option + " 매출 조회 중 에러 발생");
			        }
			    }
			    
			    // 전체 매출 조회
			    int totalSales = gdao.getTotalSales();
			    if (totalSales != -1) {
			        System.out.println("---------------------");
			        System.out.println("총 매출: " + totalSales + "원");
			        System.out.println("---------------------");
			    } else {
			        System.out.println("전체 매출 조회 중 에러 발생");
			    }
			}
